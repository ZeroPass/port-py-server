...
def main():
    ...
    tvUidDisallowedGetChl = UserId('Obi Wan Kenobi')
    tvUidDisallowedReg = UserId('crt.vavros')
    ...
    try:
        ...
        try:
            print(f'Requesting challenge from server as disallowed user {tvUidDisallowedGetChl} ...')
            requestChallenge(url, tvUidDisallowedGetChl)
            raise AssertionError('Registration with disallowed user succeed!')
        except Exception as e:
            print(f'Server returned error: {e}\n')
            assert str(e) == f"{{'code': 401, 'message': '{tvUidDisallowedGetChl} is not allowed to call proto.get_challenge'}}"

        try:
            print(f'Trying to register new user with disallowed user {tvUidDisallowedReg} ...')
            requestRegister(url, tvUidDisallowedReg, sod, dg15, sigc.id, csigs)
            raise AssertionError('Registration with disallowed user succeed!')
        except Exception as e:
            print(f'Server returned error: {e}\n')
            assert str(e) == f"{{'code': 401, 'message': '{tvUidDisallowedReg} is not allowed to register'}}"
